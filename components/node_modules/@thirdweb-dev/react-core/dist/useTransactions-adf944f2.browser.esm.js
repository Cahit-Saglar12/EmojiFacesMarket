...
const cacheKeys = {
  ...
  contract: {
    ...
    smartWallet: {
      signers: contractAddress => createContractCacheKey(contractAddress, ["smartWallet", "signers"])
    },
    app: {
...
function useSmartWallets(contract) {
  ...
  return useQueryWithNetwork(cacheKeys.contract.smartWalletFactory.getAll(contractAddress), () => {
    requiredParamInvariant(contract, "No Contract instance provided");
    invariant(contract.smartWalletFactory.getAllAccounts, "Contract instance does not support contract.smartWalletFactory.getAllAccounts");
 ...
 * @beta
 */
function useSmartWalletSigners(contract) {
  const contractAddress = contract?.getAddress();
  return useQueryWithNetwork(cacheKeys.contract.smartWallet.signers(contractAddress), () => {
    requiredParamInvariant(contract, "No Contract instance provided");
    invariant(contract.smartWallet.getSignersWithRestrictions, "Contract instance does not support contract.smartWallet.getSignersWithRestrictions");
 ...
 * @beta
 */
function useSetSmartWalletSigners(contract) {
  const activeChainId = useSDKChainId();
  const contractAddress = contract?.getAddress();
  const queryClient = useQueryClient();
